"""Hadamard-MLP variant: FFN replaced by fixed +-1 Walsh-Hadamard + learned diagonals."""
...
def hada(D_):
    import numpy as _np
    Hm=_np.array([[1.0]],_np.float32)
    while Hm.shape[0]<D_: Hm=_np.block([[Hm,Hm],[Hm,-Hm]])
    return jnp.asarray(Hm/_np.sqrt(D_))
...
np.savez("student_hada.npz", **{k: np.asarray(v) for k, v in
