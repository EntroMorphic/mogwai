...
| `semhash_probe.c` | learned semantic-hash proof: trains a tiny integer multiclass projection from the shipped exemplar labels, then encodes runtime query/candidate descriptions through the same learned map. Host-only diagnostic. `c/bin/semhash_probe --demo`; built-in red team: `c/bin/semhash_probe --redteam` |
| `runtime_choice_eval.c` | runtime-choice dataset evaluator: frozen tagged cases with arbitrary candidate choices, scored five ways (`raw_direct`, `raw_neighborhood`, polarity-aware `semhash_direct`, polarity-aware `semhash_neighborhood`, `residual_combo`) and reported accuracy, commit precision, learned coverage, wrong acts, misses, margin, collisions, OOD gates, per-query knownness, reachability failures, and polarity failures. Host-only diagnostic: `c/bin/runtime_choice_eval`; atomic breakdown: `c/bin/runtime_choice_eval --details`; topology-free factor floor sweep: `c/bin/runtime_choice_eval --floor`; semhash code-width sweep: `c/bin/runtime_choice_eval --bit-floor`; bit-width failure forensic: `c/bin/runtime_choice_eval --bit-forensic`; 20-bit candidate-fit coefficient sweep: `c/bin/runtime_choice_eval --fit-sweep`; 21-bit leave-one-out bit-importance sweep: `c/bin/runtime_choice_eval --bit-loo`; blind Holdout A with attribution: `c/bin/runtime_choice_eval --holdout`; combinatorial Holdout B: `c/bin/runtime_choice_eval --holdout-b`; unseen-referent Holdout C: `c/bin/runtime_choice_eval --holdout-c`; built-in red teams: `c/bin/runtime_choice_eval --redteam`, `c/bin/runtime_choice_eval --holdout-redteam`, `c/bin/runtime_choice_eval --holdout-b-redteam`, and `c/bin/runtime_choice_eval --holdout-c-redteam` |
| `mogwai_method_build.c` | writes the tracked second-domain method artifact, `results/log_triage.mog1`. `c/bin/mogwai_method_build [out.mog1]` |
| `mogwai_method_eval.c` | host-only Mogwai Method prototype on the second domain, device log triage. It reads a bounded `MOG1` wire artifact, parses/validates that byte image, then red-teams typed `mog_decision_t` output, refusal reasons, heuristic buckets and an operating-curve sweep. `c/bin/mogwai_method_eval results/log_triage.mog1`; per-case output: `c/bin/mogwai_method_eval --details results/log_triage.mog1`; built-in red team: `c/bin/mogwai_method_eval --redteam` |
...
| `mogwai_method_guard.c` | corruption guard for the `MOG1` method artifact parser: good parse, bad magic, truncation, count overflow, threshold drift, invalid negative flag and unterminated text all pinned. `c/bin/mogwai_method_guard results/log_triage.mog1` |
