...
| `runtime_choice_eval.c` | runtime-choice dataset evaluator: fixed tagged cases with arbitrary candidate choices, scored four ways (`raw_direct`, `raw_neighborhood`, `semhash_direct`, `semhash_neighborhood`) and reported accuracy, wrong acts, misses, margin, collisions, reachability failures, and polarity failures. Host-only diagnostic: `c/bin/runtime_choice_eval`; built-in red team: `c/bin/runtime_choice_eval --redteam` |
