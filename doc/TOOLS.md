...
| `mogwai_method_eval.c` | host-only Mogwai Method prototype on the second domain, device log triage. It compiles source cases into a bounded `MOG1` wire artifact, parses/validates that byte image, then red-teams typed `mog_decision_t` output, refusal reasons, heuristic buckets and an operating-curve sweep. Not yet a production blob. `c/bin/mogwai_method_eval`; per-case output: `c/bin/mogwai_method_eval --details`; built-in red team: `c/bin/mogwai_method_eval --redteam` |
