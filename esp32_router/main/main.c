...
static router_t R;
static rindex2 IX;
static const rvec *TI;          /* = IX.mask: the 32-byte active masks */
static uint32_t NREF; static const uint8_t *REFP;

/* v2 accessors. The index is mask + sparse sign-exception stream (router.h);
 * these keep the experiment code below reading like the v1 version did. */
#define BMASK(i) (IX.mask + (size_t)(i) * RWORDS)
#define BEP(i)   (IX.epos + IX.eoff[i])
#define BEN(i)   ((int)(IX.eoff[(i) + 1] - IX.eoff[i]))

static int load(void) {
    int rc = r_parse2(&R, &IX, blob_start, (size_t)(blob_end - blob_start));
    if (rc) return rc;
    TI = (const rvec *)(const void *)IX.mask;
...
static int route(const char *txt, int *score_out) {
    ...
    for (uint32_t i = 0; i < R.n_index; i++) {
        int s = t_score_ex(q.m, Eq, nq, BMASK(i), BEP(i), BEN(i), aa, IX.act[i]);
...
static uint32_t bench(const char *label, const tvec *q, const uint8_t *Eq, int nq,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        switch (mode) {
        case 0: {   /* touch only: read all 32 bytes, no popcount */
            uint32_t s = 0;
            for (int w = 0; w < RWORDS; w++) s += b[w];
            acc += (int)s; break; }
        case 1: {   /* t_dot only: the real popcounts, no normalisation */
            acc += t_dot_ex(q->m, Eq, nq, b, BEP(i), BEN(i)); break; }
        case 2: {   /* t_dot + t_active(b): the recompute act[] exists to avoid */
            acc += t_dot_ex(q->m, Eq, nq, b, BEP(i), BEN(i)) - t_active_m(b); break; }
...
typedef struct { const tvec *q; const uint8_t *Eq; int nq;
...
static void scan_range(job_t *j) {
    ...
    for (uint32_t i = j->lo; i < j->hi; i++) {
        int s = t_score_ex(j->q->m, j->Eq, j->nq, TI[i].w, BEP(i), BEN(i),
                           j->aa, IX.act[i]);
        if (s > best) { best = s; bi = i; }
    }
    j->best = best; j->bi = bi;
...
static int route_mt(const char *txt, int *score_out) {
    ...
    g_job.q = &q; g_job.Eq = Eq; g_job.nq = nq; g_job.aa = aa; g_job.lo = mid; g_job.hi = R.n_index;
    __atomic_thread_fence(__ATOMIC_RELEASE);
    xTaskNotifyGive(g_worker);
    job_t mine = { &q, Eq, nq, aa, 0, mid, 0, 0 };
    scan_range(&mine);
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
...
static int scan_n(const tvec *q, const uint8_t *Eq, int nq, int aa, uint32_t n) {
    int best = -(1 << 28);
    for (uint32_t i = 0; i < n; i++) {
        int s = t_score_ex(q->m, Eq, nq, TI[i].w, BEP(i), BEN(i), aa, IX.act[i]);
        if (s > best) best = s;
    }
    return best;
}
static int scan_n2(const tvec *q, const uint8_t *Eq, int nq, int aa, uint32_t n) {
    uint32_t mid = n / 2;
    g_job.q = q; g_job.Eq = Eq; g_job.nq = nq; g_job.aa = aa; g_job.lo = mid; g_job.hi = n;
    __atomic_thread_fence(__ATOMIC_RELEASE);
    xTaskNotifyGive(g_worker);
    job_t mine = { q, Eq, nq, aa, 0, mid, 0, 0 };
    scan_range(&mine);
...
static int64_t reps(const char *label, int cores, uint32_t n) {
    ...
    for (int r = 0; r < NREP; r++) {
        ...
        for (uint32_t i = 0; i < NREF; i++) {
            ...
            if (cores == 1) scan_n(&q, Eq, nq, aa, n); else scan_n2(&q, Eq, nq, aa, n);
 ...
 *
 * The earlier profile found "touch 32B only" costs the same cached or uncached
...
static void dram_vs_flash(void) {
    ...
    t0 = esp_timer_get_time();
    for (int r = 0; r < 4; r++) for (uint32_t i = 0; i < N; i++) s += TI[i].w[0] + TI[i].w[7];
    int64_t tf2 = esp_timer_get_time() - t0;
    t0 = esp_timer_get_time();
    for (int r = 0; r < 4; r++) for (uint32_t i = 0; i < N; i++) s += D[i].w[0] + D[i].w[7];
...
static void bulk_and_pipeline(void) {
    ...
    rvec *b0 = malloc(CH * sizeof(rvec)), *b1 = malloc(CH * sizeof(rvec));
    ...
    int64_t t0 = esp_timer_get_time();
    int best = -(1 << 28); uint32_t bi = 0;
    rvec *cur = b0, *nxt = b1; uint16_t *ca = a0, *na = a1;
    uint32_t done = 0, first = CH < R.n_index ? CH : R.n_index;
    memcpy(cur, TI, first * sizeof(rvec)); memcpy(ca, IX.act, first * 2);
    while (done < R.n_index) {
        uint32_t n = R.n_index - done; if (n > CH) n = CH;
        uint32_t ahead = R.n_index - (done + n); if (ahead > CH) ahead = CH;
        if (ahead) { memcpy(nxt, &TI[done + n], ahead * sizeof(rvec));
                     memcpy(na, &IX.act[done + n], ahead * 2); }
        for (uint32_t i = 0; i < n; i++) {
            int s = t_score_ex(q.m, Eq, nq, cur[i].w, BEP(done+i), BEN(done+i), aa, ca[i]);
            if (s > best) { best = s; bi = done + i; }
        }
        done += n;
        rvec *tb = cur; cur = nxt; nxt = tb;
...
static uint64_t fold_mask(const uint32_t *m) {
    uint64_t f = 0;
    for (int d = 0; d < RD; d++)
        if (m[d >> 5] & (1u << (d & 31))) f |= 1ULL << (d >> 2);
    return f;
}
static void two_stage(void) {
    ...
    for (uint32_t i = 0; i < R.n_index; i++) SIG64[i] = fold_mask(TI[i].w);
    ...
    printf("  -- pure SIZE sweep: identical content, only N varies --\n");
    {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            snprintf(lb, sizeof lb, "N=%-6lu (%3lu KB)", (unsigned long)sz[i],
                     (unsigned long)(sz[i] * sizeof(rvec) / 1024));
...
static void profile(const char *txt) {
    ...
    printf("\n  --- FULL index (%lu vectors, %lu KB — streams through a 32KB cache) ---\n",
           (unsigned long)R.n_index, (unsigned long)(R.n_index * sizeof(rvec) / 1024));
    bench("touch 32B only (no arithmetic)", &q, Eq, nq, aa, R.n_index, 0);
    bench("+ popcount dot (t_dot)", &q, Eq, nq, aa, R.n_index, 1);
    bench("+ t_active(b) recompute", &q, Eq, nq, aa, R.n_index, 2);
    bench("+ integer divide (full t_score)", &q, Eq, nq, aa, R.n_index, 3);
    printf("\n  --- SMALL index (%lu vectors, %lu KB — fits in cache) ---\n",
           (unsigned long)small, (unsigned long)(small * sizeof(rvec) / 1024));
    bench("touch 32B only (no arithmetic)", &q, Eq, nq, aa, small, 0);
...
void app_main(void) {
    ...
    printf("resident    : %lu bytes of index (in flash, not RAM)\n",
           (unsigned long)(R.n_index * sizeof(rvec)));
