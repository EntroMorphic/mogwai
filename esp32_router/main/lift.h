...
#define LIFT_CHUNK_VECS 128                  /* 8 KB per chunk */
#define LIFT_MAXCHUNK   512                  /* 65,536 vectors */
...
typedef struct {
    const tvec    *ch[LIFT_MAXCHUNK];        /* chunk -> SRAM copy, or into flash */
    const uint16_t *act;                     /* uint16_t: NEVER eligible for IRAM */
    uint32_t n_index, nch;
