...
96-check regression suite runs in 25 s - most of which is an exhaustive 2^32
...
## Build and flash
    ...
    make regress        # full host regression (96 checks) — run after any structural change
...
## Repo map
    ...
    scripts/           fetch.sh (curl only, pinned revisions, verifies checksums),
                       regress.sh (96 checks), mutate.sh
...
state of the tree. Run `make regress` after any structural change: 96 checks,
25 seconds.

CI runs the same 96 checks on Linux/GCC and builds the VALIDATION, PRODUCT and
