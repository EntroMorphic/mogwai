...
67-check regression suite runs in 19 s - most of which is an exhaustive 2^32
...
## Build and flash
    ...
    make regress        # full host regression (67 checks) — run after any structural change
...
## Repo map
    ...
    scripts/           fetch.sh (curl only), regress.sh (67 checks), mutate.sh
...
state of the tree. Run `make regress` after any structural change: 67 checks,
19 seconds.

CI runs the same 67 checks on Linux/GCC and builds both ESP32 firmwares from
