...
61-check regression suite runs in 19 s - most of which is an exhaustive 2^32
...
## Repo map
    ...
    scripts/           fetch.sh (curl only), regress.sh (61 checks), mutate.sh
...
state of the tree. Run `make regress` after any structural change: 61 checks,
19 seconds.

CI runs the same 61 checks on Linux/GCC and builds both ESP32 firmwares from
